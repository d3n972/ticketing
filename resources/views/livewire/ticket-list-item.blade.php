...
>
    <span
        class="h-1/2 mb-1 mr-5 px-2 rounded {{ $issue->severity()->first()->getCss() }}">{{ $issue->severity()->first()->name[0] }}</span>
    <div class="flex-1 gap-4 grid hidden md:block">
            ...
            <div class="grid grid-cols-3">
                ...
                <div class="flex gap-4 ml-auto mr-0">
                    <span>
                        ...
                        >
                            <g>
                                <path
                                    class="icon-comment"
                                    d="M13.23,2H2.77A1.77,1.77,0,0,0,1,3.77v7.44A1.77,1.77,0,0,0,2.77,13H3v3l4.16-3h6.06A1.77,1.77,0,0,0,15,11.21V3.77A1.77,1.77,0,0,0,13.23,2Zm.37,9.21a.38.38,0,0,1-.37.37H2.77a.38.38,0,0,1-.37-.37V3.77a.38.38,0,0,1,.37-.37H13.23a.38.38,0,0,1,.37.37Z"
                                ></path>
                            </g>
                        </svg>
                    </span>
                    <span>
                        ...
                        >
                            <path
                                d="M1 15h1.8V6H1zm12.6-9.38H10L10.64 4a3.31 3.31 0 0 0 .21-1.76 2.72 2.72 0 0 0-.64-1.37 2.77 2.77 0 0 0-.67-.56A1.17 1.17 0 0 0 8.65 0 1 1 0 0 0 8 .66c-.11.29-.22.59-.34.88l-.57 1.4L4 6v9h7.11a1.82 1.82 0 0 0 1.61-1l1.94-3.68A3 3 0 0 0 15 8.94V7.07a1.42 1.42 0 0 0-1.39-1.45zm0 3.32a1.58 1.58 0 0 1-.18.73l-1.93 3.68a.46.46 0 0 1-.38.25H5.4v-7l2.67-2.67a1.37 1.37 0 0 0 .3-.45c.13-.29.44-1 .72-1.76a1.46 1.46 0 0 1 .26 1.7L8 7l5.6.05z"
                            ></path>
                        </svg>
                    </span>
        ...
        >{!! $issue->getContent() !!}</div>
    </div>
    <div class="flex-1 gap-4 grid  md:hidden">
        ...
        <div class="flex-1 grid grid-rows-2">
            ...
            <div class="hidden md:divide-fields gap-x-0 gap-y-2 grid grid-cols-3 leading-snug text-sm">
                <span class="">
                    {{ $issue->severity()->first()->name }}
                </span>
                <span class="">[type]</span>
            ...
            <div class="mt-2">
                <span class="">{{ $issue->author()->first()->name }}</span>
                <span class="">{{ $issue->getTimeHuman($issue->updated_at) }}</span>
            </div>
        </div>
        <div
            class="border border-gray-800 content max-h-96 overflow-auto p-5 w-max"
            x-show="open"
            style="display: none;"
            @click.away="open = false"
        >{!! $issue->getContent() !!}</div>
